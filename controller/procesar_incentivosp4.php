...
foreach ($_POST as $day => $values) {
    foreach ($values as $id_precilla4 => $eficiencia) {
        // Escapar y validar valores
        $day_column = htmlspecialchars($day);
        $id_precilla4 = (int) $id_precilla4;
        ...
        if (in_array($day_column, $valid_columns)) {
            ...
            $check_query = "SELECT * FROM incentivop4 WHERE id_usuario = $id_precilla4";
            $result = $conexion->query($check_query);

            if ($result->num_rows > 0) {
                // Si el registro existe, actualiza el campo de eficiencia e incentivo correspondiente
                $update_query = "UPDATE incentivop4 SET $day_column = $eficiencia, $valor_column = $incentivo WHERE id_usuario = $id_precilla4";
                $conexion->query($update_query);
            } else {
                // Si el registro no existe, inserta un nuevo registro con los valores de eficiencia e incentivo
                $insert_query = "INSERT INTO incentivop4 (id_usuario, $day_column, $valor_column) VALUES ($id_precilla4, $eficiencia, $incentivo)";
...
$users_query = "SELECT id_usuario FROM incentivop4";
...
if ($users_result->num_rows > 0) {
    while ($user = $users_result->fetch_assoc()) {
        ...
        $update_total_query = "UPDATE incentivop4 SET valor = $total_incentivo WHERE id_usuario = $id_usuario";
        $conexion->query($update_total_query);
                // Actualizar la columna `valor_1` en usuario
                $update_usuario_query = "UPDATE usuario SET valor_4 = $total_incentivo WHERE id_usuario = $id_usuario";
