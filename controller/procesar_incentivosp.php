...
foreach ($_POST as $day => $values) {
    foreach ($values as $id_precilla => $eficiencia) {
        // Escapar y validar valores
        $day_column = htmlspecialchars($day);
        $id_precilla = (int) $id_precilla;
        ...
        if (in_array($day_column, $valid_columns)) {
            ...
            $check_query = "SELECT * FROM incentivop WHERE id_usuario = $id_precilla";
            $result = $conexion->query($check_query);

            if ($result->num_rows > 0) {
                // Si el registro existe, actualiza el campo de eficiencia e incentivo correspondiente
                $update_query = "UPDATE incentivop SET $day_column = $eficiencia, $valor_column = $incentivo WHERE id_usuario = $id_precilla";
                $conexion->query($update_query);
            } else {
                // Si el registro no existe, inserta un nuevo registro con los valores de eficiencia e incentivo
                $insert_query = "INSERT INTO incentivop (id_usuario, $day_column, $valor_column) VALUES ($id_precilla, $eficiencia, $incentivo)";
...
// Calcular el total de incentivo semanal para cada usuario y actualizar la columna `valor` en incentivop y `valor_1` en usuario
$users_query = "SELECT id_usuario FROM incentivop";
...
if ($users_result->num_rows > 0) {
    while ($user = $users_result->fetch_assoc()) {
        ...
        // Actualizar la columna `valor` en incentivop
        $update_total_query = "UPDATE incentivop SET valor = $total_incentivo WHERE id_usuario = $id_usuario";
        $conexion->query($update_total_query);

        // Actualizar la columna `valor_1` en usuario
        $update_usuario_query = "UPDATE usuario SET valor_1 = $total_incentivo WHERE id_usuario = $id_usuario";
